...
const findRoleFromRequest = (
  ...
  if (searchPath) {
    if (objectPath.get(req, searchPath)) {
      return objectPath.get(req, searchPath);
    }
  
    //probably at this point the search path didn't work, lets get the first object and try again
    let explode = searchPath.split(".");
    let object = explode.shift();
    searchPath = explode.join(".");
    if (req[object]) {
      let role = objectPath.get(JSON.parse(JSON.stringify(req[object])), searchPath);
      if (role) {
        return role;
      }
    }    
